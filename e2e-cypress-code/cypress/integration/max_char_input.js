describe('Textbox with max characters', () => {
  it('displays the correct remaining characters count', () => {
    ...
    cy.get('input').type(' my spaceship');

    cy.get('span').invoke('text').should('equal', '0');
  });

  it('prevents the user from typing more characters once max is exceeeded', () => {
    cy.visit('http://localhost:3000/example-2');
